...
use super::objects::{Player, Wall, Target, GameObject, Collidable, AdditionalAction};
...
impl Level {
    pub fn from_file<P: AsRef<Path> + Display>(file_name: P) -> Result<Level> {
        ...
        for line in reader.lines() {
            match state {
                ReadState::NoLines => state = ReadState::OneLine(try!(line)),
                ReadState::OneLine(s) => state = ReadState::TwoLines(s, try!(line)),
